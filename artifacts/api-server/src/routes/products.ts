...
const PRODUCT_KEYWORD_IMAGES: Array<{ re: RegExp; image: string }> = [
...
const EXACT_NAME_IMAGES: Record<string, string> = {
...
const isBlockedImageHost = (url: any) => {
  if (!url) return true;
  const normalized = String(url).toLowerCase();
  return normalized.includes('openfoodfacts.org') || normalized.includes('dummyjson.com/image');
};

const normalizeProductName = (name: any) => String(name || '').trim().toLowerCase();

const resolveProductImage = (name: any, categorySlug: any, imageUrl: any) => {
...
router.get("/", async (req, res) => {
  try {
    const { search = '', category = '', limit = '50' } = req.query as any;
