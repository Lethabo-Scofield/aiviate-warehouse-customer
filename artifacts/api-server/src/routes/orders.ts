...
const loadOrderHistory = async (client: any, orderId: any) => {
  ...
  try {
    ...
    await client.query(
      `INSERT INTO order_status_history (
       ...
       VALUES ($1, $2, $3, $4, $5)`,
      [order.id, null, String(order.status || 'confirmed'), 'Order created', (req as any).user.id]
    );
    ...
    if (candidateProductIds.length > 0) {
      ...
      validProductIdSet = new Set(existingProductsResult.rows.map((row: any) => Number(row.id)));
    }
...
router.put("/:id/status", async (req, res) => {
  ...
  try {
    ...
    const result = await client.query(
      ...
      [nextStatus, req.params.id, (req as any).user.id]
    );
    ...
    if (previousStatus !== nextStatus) {
      await client.query(
        `INSERT INTO order_status_history (
         ...
         VALUES ($1, $2, $3, $4, $5)`,
        [req.params.id, previousStatus, nextStatus, 'Status updated', (req as any).user.id]
      );
    }
  ...
  try {
    ...
    await client.query(
      `INSERT INTO order_status_history (
       ...
       VALUES ($1, $2, $3, $4, $5)`,
      [req.params.id, currentOrder.status, currentOrder.status, `Delivery address updated from "${currentOrder.shipping_address || 'not set'}" to "${trimmedAddress}"`, (req as any).user.id]
    );
